...
from core.database import save_result # Importiamo la funzione da database.py
...
class SecurityScanner:
  ...
  def run_audit(self):
   ...
   # Eseguiamo ogni test e salviamo
   for scan_func in [self.scan_dns(), self.scan_tls(), self.scan_headers(), 
                    self.scan_sensitive_files(), self.scan_bot_ai_behavior()]:
       # Queste due righe DEVONO avere la rientranza (indentazione)
       audit_data.append(scan_func)
       save_result(self.target, scan_func)

   # Scansione del CMS
   detector = CMSDetector(self.target)
   cms_result = detector.run_passive_scan()
   audit_data.append(cms_result)
   save_result(self.target, cms_result)
